...
router.post('/', async (req,res)=>{
    ...
    var waitforgetTeacherbyID = new Promise((resolve, reject) => {
        const cursor = db.collection('doctoralprograms').find({_id : mongoose.Types.ObjectId(req.body.DeleteDirectorID)});
          ...
          waitforgetTeacherbyID.then((User) => {
            try {
                 var myquery = { _id: User._id}
                       db.collection("doctoralprograms").deleteOne(myquery, function(err, res) {
                           if (err) throw err;
                       });

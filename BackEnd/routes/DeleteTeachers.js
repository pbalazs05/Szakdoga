...
router.post('/', async (req,res)=>{
    ...
    var waitforgetTeacherbyID = new Promise((resolve, reject) => {
        const cursor = db.collection('nameandemails').find({_id : mongoose.Types.ObjectId(req.body.DeleteTeacherID)});
          ...
          waitforgetTeacherbyID.then((User) => {
            try {
                 var myquery = { _id: User._id}
                       db.collection("nameandemails").deleteOne(myquery, function(err, res) {
                           if (err) throw err;
                       });
